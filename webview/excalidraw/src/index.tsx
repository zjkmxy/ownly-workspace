import { getSceneVersion, loadFromBlob, loadLibraryFromBlob } from '@excalidraw/excalidraw';
import React from 'react';
import ReactDOM from 'react-dom';
import { Base64 } from 'js-base64';

import App from './App';
import { sendChangesToVSCode, channel } from './channel';
import {
  AppState,
  BinaryFiles,
...
async function getInitialData(
  ...
  //       new Blob([contentType == 'image/png' ? content : new TextDecoder().decode(content)], {
  //         type: contentType,
  //       }),
  //       null,
  //       null
  ...
  const initialData = await loadFromBlob(
    new Blob([new TextDecoder().decode(content)], { type: 'application/json' }),
    null,
    null
  );
  return [{ ...initialData }, contentType];
...
async function getLibraryItems(libraryString: string) {
  try {
    return await loadLibraryFromBlob(new Blob([libraryString], { type: 'application/json' }));
  } catch (e) {
    channel.postMessage({
      type: 'error',
  ...
  try {
    ...
    const [initialData, initialContentType] =
      config.content.length > 0
        ? await getInitialData(new Uint8Array(config.content), config.contentType)
    ...
    ReactDOM.render(
      <React.StrictMode>
        <App
          ...
          libraryItems={config.library ? await getLibraryItems(config.library) : []}
          name={config.name}
