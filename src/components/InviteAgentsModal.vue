...
async function acceptRequest(invitee: IProfile) {
  ...
  const idx = pendingRequests.value.indexOf(invitee, 0);
  if (idx > -1) pendingRequests.value.splice(idx, 1);

  // Publish agent invitation
  try {
  ...
  Toast.success(`Invited 1 agent to workspace!`);
}
...
async function send() {
  ...
  for (const invitee of pendingInvitees.value) {
    try {
      ...
      await wksp.value.invite.tryAgentInvite(invitee, inviteChannel.value, inviteUrl.value);
    } catch (err) {
      Toast.error(`Failed to invite ${invitee.name}: ${err}`);
      return; // rare
    }
